...
impl Sv39PageTable {
    ...
    pub fn map_page(&mut self, va: u64, pa: u64, flags: u64) {
        ...
        for level in [2, 1] {
            ...
            let pte = self.nodes.get_mut(&current_ppn).unwrap().entries[index];
    ...
    pub fn translate(&self, va: u64) -> TranslateResult {
        ...
        for level in [2, 1, 0] {
            let index = Self::extract_vpn(va, level);
            let pte = self.nodes[&current_ppn].entries[index];
            let offset = va & ((1u64 << (12 + level * 9)) - 1);
            ...
            if pte & (PTE_R | PTE_W | PTE_X) != 0 {
                ...
                return TranslateResult::Ok(ppn << (12 + 9 * level) | offset);
            }
            current_ppn = pte >> PPN_SHIFT;
        }
