...
impl<T> RwLock<T> {
    ...
    pub fn read(&self) -> RwLockReadGuard<'_, T> {
        loop {
            ...
            if (s & WRITER_HOLDING != 0) || (s & WRITER_WAITING != 0) || (s & READER_MASK == READER_MASK) {
                core::hint::spin_loop();
                continue;
            }
    ...
    pub fn write(&self) -> RwLockWriteGuard<'_, T> {
        ...
        loop {
            ...
            match self.state.compare_exchange(WRITER_WAITING, WRITER_HOLDING, Ordering::Release, Ordering::Acquire) {
